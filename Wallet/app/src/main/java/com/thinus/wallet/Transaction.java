...
public class Transaction {
    ...
    {
        return CategoryListActivity.getCategoryName(_categoryId);
    }

    public String getCategoryNameType()
    {
        return CategoryListActivity.getCategoryNameType(_categoryId);
    }
    ...
    public void Save()
    {
        try {
            ...
            TransactionListActivity.db.execSQL("INSERT INTO [transaction] VALUES('" + _id + "','" + _amount + "','" + _categoryId + "','" + _account + "','" + _description + "','" + dateFormat.format(_date) + "','" + _reference + "','" + _income_expense + "');");
        } catch (Exception ex) {
            _error = ex.toString();
        }
    }
    ...
    public void Update()
    {
        try {
            ...
            TransactionListActivity.db.execSQL("UPDATE [transaction] SET amount = '" + _amount + "', categoryid = '" + _categoryId + "', account = '" + _account + "', description = '" + _description + "', date = '" + dateFormat.format(_date) + "', reference = '" + _reference + "', income_expense = '" + _income_expense + "' WHERE id = " + _id + ";");
        } catch (Exception ex) {
            _error = ex.toString();
        }
    }
    ...
    {
        try {
            TransactionListActivity.db.execSQL("DELETE FROM [transaction] WHERE id = " + _id + ";");
        } catch (Exception ex) {
            _error = ex.toString();
        }
    }
